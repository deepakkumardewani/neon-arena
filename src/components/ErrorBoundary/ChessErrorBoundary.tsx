...
function ChessErrorFallback({ error, reset }: FallbackProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3">
        <button
          type="button"
          onClick={() => void navigate("/play/chess")}
...
export function ChessErrorBoundary({ children }: { children: ReactNode }) {
  return (
    <ErrorBoundary fallback={(props) => <ChessErrorFallback {...props} />}>
      {children}
    </ErrorBoundary>
  );
}
