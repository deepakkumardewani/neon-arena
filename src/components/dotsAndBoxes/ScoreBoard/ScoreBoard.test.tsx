...
describe("DotsScoreBoard", () => {
  ...
  it("renders P1 and P2 badges", () => {
    const { getByText } = render(<DotsScoreBoard {...baseProps} p1Label="Alice" p2Label="Bob" />);
    expect(getByText("Alice")).toBeDefined();
    expect(getByText("Bob")).toBeDefined();
  });

  it("displays correct scores", () => {
    const { getAllByText } = render(<DotsScoreBoard {...baseProps} scores={{ 1: 4, 2: 3 }} />);
  ...
  it("renders bonus-turn tag when lastClaimedBoxes is non-empty", () => {
    const { getByText } = render(
      <DotsScoreBoard {...baseProps} lastClaimedBoxes={[{ row: 0, col: 0 }]} currentPlayer={1} />,
    );
    expect(getByText("+1 turn")).toBeDefined();
  });

  it("does not render bonus-turn tag when lastClaimedBoxes is empty", () => {
    const { queryByText } = render(<DotsScoreBoard {...baseProps} lastClaimedBoxes={[]} />);
